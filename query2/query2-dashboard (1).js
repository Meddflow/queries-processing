...
export async function init(uri, onStatus = () => {}) {
  ...
  const patients = await mongoAggregate(uri, db, 'PATIENTS', [
    { $match: { [CONFIG.fields.patientId]: { $ne: null } } },
    { $project: {
        _id: 0,
        [CONFIG.fields.patientId]: 1,
        [CONFIG.fields.ageField]:  1,
        [CONFIG.fields.dobField]:  1,
        [CONFIG.fields.agebandField]: 1,
        [CONFIG.fields.postcode]:  1,
    }},
  ]);
  ...
  const appts = await mongoAggregate(uri, db, 'APPOINTMENTS', [
    { $match: {
        RECORDSTATUS: 1,
        APPOINTMENTDATE: { $gte: new Date('2011-01-01'), $lte: new Date('2025-12-31') },
        [CONFIG.fields.patientId]: { $ne: null },
    }},
    { $project: {
        _id: 0,
        [CONFIG.fields.patientId]: 1,
...
function buildEnrichedPatients(patients, history, appts, phnMap) {
  ...
  for (const r of appts) {
    const pid = String(r[CONFIG.fields.patientId] ?? '');
    if (!pid) continue;
    const d = new Date(r[CONFIG.fields.apptDate]);
    if (isNaN(d)) continue;
  ...
  for (const p of patients) {
    const pid = String(p[CONFIG.fields.patientId] ?? '');
    if (!pid) continue;
...
function computeAge(patient) {
  ...
  const age = patient[CONFIG.fields.ageField];
